...
bot_token = "5747791215:AAFF4rrM8ZOz2BgJUhSgQ7BATvTHoz5YJtM"
  ...
  await app.reply_to(
      ...
      reply_markup=Keyboard([
          ...
          [
              ...
              Button("- SoundCloud -",
                     callback_data="soundcloud")  # @BENN_DEV & @BENfiles
          ],
...
def instagram(url):
  ...
  response = requests.post('https://reelsaver.net/api/instagram',
                           headers=headers,
                           data=data).json()
...
@app.callback_query_handler(func=lambda callback: callback.data == "instagram")
...
@app.callback_query_handler(func=lambda callback: callback.data == "pintrest")
...
@app.callback_query_handler(func=lambda callback: callback.data == "snapchat")
async def snap_vid(callback: CallbackQuery):
  ...
  await app.delete_message(user_id, answer.output.id)  # @BENN_DEV & @BENfiles
...
def tiktok(url):
  headers = {
      ...
      'cors',
      'sec-fetch-site':
      'same-origin',  # @BENN_DEV & @BENfiles
      'user-agent':
      'Mozilla/5.0 (Linux; Android 12; M2004J19C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/105.0.0.0 Mobile Safari/537.36',
  }
  ...
  response = requests.post('https://ssstik.io/abc', headers=headers,
                           data=data).text
  try:
    ...
    return {"success": False}
  urls = response.split(
      '<div class="flex-1 result_overlay_buttons pure-u-1 pure-u-sm-1-2">')[1]
...
@app.callback_query_handler(func=lambda callback: callback.data == "tiktok")
async def tik_vid(callback: CallbackQuery):
  ...
  await app.send_video(chat_id=user_id,
...
@app.callback_query_handler(func=lambda callback: callback.data == "youtube")
...
async def download(callback: CallbackQuery):
  ...
  thumbnail = info["thumbnail"]["thumbnails"][-1][
      "url"]  # @BENN_DEV & @BENfiles
  ...
  if stream.type.startswith(('video', "audio")) and stream.filesize_mb > 100:
    ...
    await app.edit_message_text(chat_id=callback.message.chat.id,
...
def soundcloud(url):
  params = {"url": url}
  response = requests.post("https://api.downloadsound.cloud/track",
                           json=params).json()
...
@app.callback_query_handler(func=lambda callback: callback.data == "soundcloud"
                            )
...
@app.message_handler(
    func=lambda message: others["options"][
        "forward_from_users"] and message.from_user.id not in admins,
    chat_types=["private"],
  ...
  for adminn in admins:
    try:
      await app.forward_message(
          ...
          message_id=message.id  # @BENN_DEV & @BENfiles
      )
    except:
      continue
  await app.reply_to(message, "-> تم إرسال رسالتك للمطور!")
